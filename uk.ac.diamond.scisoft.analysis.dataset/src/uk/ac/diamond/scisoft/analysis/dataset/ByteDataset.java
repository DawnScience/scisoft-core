...
public class ByteDataset extends AbstractDataset {
	...
	ByteDataset setSlicedView(Dataset view, Dataset d) {
		...
		while (biter.hasNext()) {
			data[biter.aIndex] = (byte) d.getElementLongAbs(biter.bIndex); // GET_ELEMENT_WITH_CAST
		}
		return this;
	}
	...
	public ByteDataset isubtract(final Object b) {
		...
		final BroadcastIterator it = new BroadcastIterator(this, bds);
		while (it.hasNext()) {
			data[it.aIndex] = (byte) (it.aDouble - it.bDouble); // INT_USE // ADD_CAST
		}
		setDirty();
		return this;
	}
	...
	public ByteDataset iremainder(final Object b) {
		...
		while (it.hasNext()) {
				try {
				data[it.aIndex] %= (byte) it.bDouble; // ADD_CAST // INT_EXCEPTION
