...
public class CompoundDoubleDataset extends AbstractCompoundDataset {
	 ...
	 */
	protected static final Logger compoundLogger = LoggerFactory.getLogger(CompoundDoubleDataset.class);
		...
		if (obj instanceof IDataset) {
			...
			if (ds instanceof Dataset) {
				...
				while (iter.hasNext() && itd.hasNext()) {
					double[] vr = toDoubleArray(ads.getObjectAbs(itd.index), isize); // PRIM_TYPE // CLASS_TYPE
					for (int i = 0; i < isize; i++) {
				...
				IndexIterator iter = getIterator();
				while (iter.hasNext() && itd.hasNext()) {
					double[] vr = toDoubleArray(ds.getObject(pos), isize); // PRIM_TYPE // CLASS_TYPE
					for (int i = 0; i < isize; i++) {
						data[iter.index + i] = vr[i]; // PRIM_TYPE
					}
				}
	...
	public Object getObject(final int i) {
		return getDoubleArray(i); // CLASS_TYPE
	}

	@Override
	public Object getObject(final int i, final int j) {
		return getDoubleArray(i, j); // CLASS_TYPE
	}
	 ...
	 */
	public double[] getDoubleArray(final int i) { // CLASS_TYPE // PRIM_TYPE
		return (double[]) getObjectAbs(get1DIndex(i)); // PRIM_TYPE
	}
	 ...
	 * @return item in given position
	 */
	public double[] getDoubleArray(final int i, final int j) { // CLASS_TYPE // PRIM_TYPE
		return (double[]) getObjectAbs(get1DIndex(i, j)); // PRIM_TYPE
	}
	...
	public double[] getDoubleArray(final int... pos) { // CLASS_TYPE // PRIM_TYPE
		return (double[]) getObjectAbs(get1DIndex(pos)); // PRIM_TYPE
	}
