...
public class CompoundIntegerDataset extends AbstractCompoundDataset {
	...
	public CompoundIntegerDataset iadd(final Object b) {
		...
		if (bds.getElementsPerItem() == 1) {
			while (it.hasNext()) {
				...
				for (int i = 1; i < isize; i++)
					data[it.aIndex + i] += (int) it.bDouble; // ADD_CAST
			}
		} else {
			while (it.hasNext()) {
				data[it.aIndex] += it.bDouble;
				for (int i = 1; i < isize; i++)
					data[it.aIndex + i] += (int) bds.getElementLongAbs(it.bIndex + i); // GET_ELEMENT_WITH_CAST
			}
		}
		setDirty();
		return this;
	...
	public CompoundIntegerDataset isubtract(final Object b) {
		...
		if (bds.getElementsPerItem() == 1) {
			while (it.hasNext()) {
				data[it.aIndex] -= it.bDouble;
				for (int i = 1; i < isize; i++)
					data[it.aIndex + i] = (int) (it.aDouble - it.bDouble); // INT_USE // ADD_CAST
			}
		} else {
			while (it.hasNext()) {
				data[it.aIndex] -= it.bDouble;
				for (int i = 1; i < isize; i++)
					data[it.aIndex + i] = (int) (data[it.aIndex + i] - bds.getElementDoubleAbs(it.bIndex + i)); // INT_USE // ADD_CAST
			}
		}
		setDirty();
		return this;
	...
	public CompoundIntegerDataset idivide(final Object b) {
		...
		if (bds.getElementsPerItem() == 1) {
			while (it.hasNext()) {
				...
				for (int i = 0; i < isize; i++)
					data[it.aIndex + i] = (int) (data[it.aIndex + i] / db); // INT_USE // ADD_CAST
				} // INT_USE
			}
		} else {
			while (it.hasNext()) {
	...
	public CompoundIntegerDataset iremainder(final Object b) {
		...
		if (bds.getElementsPerItem() == 1) {
			while (it.hasNext()) {
				final int db = (int) it.bDouble; // PRIM_TYPE // ADD_CAST
