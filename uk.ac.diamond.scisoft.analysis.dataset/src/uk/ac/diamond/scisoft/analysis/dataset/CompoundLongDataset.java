...
public class CompoundLongDataset extends AbstractCompoundDataset {
		...
		if (obj instanceof IDataset) {
			...
			if (ds instanceof Dataset) {
				...
				while (iter.hasNext() && itd.hasNext()) {
					long[] vr = toLongArray(ads.getObjectAbs(itd.index), isize); // PRIM_TYPE // CLASS_TYPE
					for (int i = 0; i < isize; i++) {
				...
				IndexIterator iter = getIterator();
				while (iter.hasNext() && itd.hasNext()) {
					long[] vr = toLongArray(ds.getObject(pos), isize); // PRIM_TYPE // CLASS_TYPE
					for (int i = 0; i < isize; i++) {
						data[iter.index + i] = vr[i]; // PRIM_TYPE
					}
				}
	...
	public void set(final Object obj, final int i) {
		setItem(toLongArray(obj, isize), i); // CLASS_TYPE
	}

	@Override
	public void set(final Object obj, final int i, final int j) {
		setItem(toLongArray(obj, isize), i, j); // CLASS_TYPE
	}
	...
	public CompoundLongDataset iadd(final Object b) {
		...
		if (bds.getElementsPerItem() == 1) {
			while (it.hasNext()) {
				...
				for (int i = 1; i < isize; i++)
					data[it.aIndex + i] += (long) it.bDouble; // ADD_CAST
			}
		} else {
			while (it.hasNext()) {
				data[it.aIndex] += it.bDouble;
				for (int i = 1; i < isize; i++)
					data[it.aIndex + i] += bds.getElementLongAbs(it.bIndex + i); // GET_ELEMENT_WITH_CAST
			}
		}
		setDirty();
		return this;
	...
	public CompoundLongDataset isubtract(final Object b) {
		...
		if (bds.getElementsPerItem() == 1) {
			while (it.hasNext()) {
				data[it.aIndex] -= it.bDouble;
				for (int i = 1; i < isize; i++)
					data[it.aIndex + i] = (long) (it.aDouble - it.bDouble); // INT_USE // ADD_CAST
			}
		} else {
			while (it.hasNext()) {
				data[it.aIndex] -= it.bDouble;
				for (int i = 1; i < isize; i++)
					data[it.aIndex + i] = (long) (data[it.aIndex + i] - bds.getElementDoubleAbs(it.bIndex + i)); // INT_USE // ADD_CAST
			}
		}
		setDirty();
		return this;
	...
	public CompoundLongDataset idivide(final Object b) {
		...
		if (bds.getElementsPerItem() == 1) {
			while (it.hasNext()) {
				...
				for (int i = 0; i < isize; i++)
					data[it.aIndex + i] = (long) (data[it.aIndex + i] / db); // INT_USE // ADD_CAST
				} // INT_USE
			}
		} else {
			while (it.hasNext()) {
	...
	public CompoundLongDataset iremainder(final Object b) {
		...
		if (bds.getElementsPerItem() == 1) {
			while (it.hasNext()) {
				final long db = (long) it.bDouble; // PRIM_TYPE // ADD_CAST
			...
			while (it.hasNext()) {
				for (int i = 0; i < isize; i++) {
				try {
						data[it.aIndex + i] %= bds.getElementLongAbs(it.bIndex + i); // GET_ELEMENT // INT_EXCEPTION
