...
public class CompoundByteDataset extends AbstractCompoundDataset {
	 ...
	 * @param dataset
	 */
	public CompoundByteDataset(final int itemSize, final boolean repeat, final AbstractDataset dataset) {
		isize = itemSize;
		size = dataset.size;
		shape = dataset.shape.clone();
		name = new String(dataset.name);
	...
	public byte[] getByteArray(final int... pos) { // CLASS_TYPE // PRIM_TYPE
		...
		return (byte[]) getObjectAbs(n); // PRIM_TYPE
	}
	...
	@Override
	public CompoundByteDataset setByBoolean(final Object o, BooleanDataset selection) {
	...
	@Override
	public CompoundByteDataset setByIndex(final Object o, IntegerDataset index) {
		if (o instanceof AbstractDataset) {
			AbstractDataset ds = (AbstractDataset) o;
	...
	@Override
	public CompoundByteDataset setSlice(final Object o, final int[] start, final int[] stop, final int[] step) {
		SliceIterator siter;
		if (o instanceof AbstractDataset) {
			...
			if (ds instanceof AbstractCompoundDataset) {
				...
				while (siter.hasNext() && oiter.hasNext()) {
					for (int i = 0; i < isize; i++)
						data[siter.index + i] = (byte) ds.getElementLongAbs(oiter.index + i); // GET_ELEMENT_WITH_CAST
				}
			} else {
				while (siter.hasNext() && oiter.hasNext()) {
					data[siter.index] = (byte) ds.getElementLongAbs(oiter.index); // GET_ELEMENT_WITH_CAST
					for (int i = 1; i < isize; i++)
	...
	public void copyItemsFromAxes(final int[] pos, final boolean[] axes, final AbstractDataset dest) {
		byte[] ddata = (byte[]) dest.odata; // PRIM_TYPE
