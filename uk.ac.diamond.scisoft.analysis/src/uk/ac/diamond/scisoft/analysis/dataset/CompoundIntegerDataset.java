...
public class CompoundIntegerDataset extends AbstractCompoundDataset {
	 ...
	 * @param datasets
	 */
	public CompoundIntegerDataset(final AbstractDataset... datasets) {
	 ...
	 * @param dataset
	 */
	public CompoundIntegerDataset(final int itemSize, final boolean repeat, final AbstractDataset dataset) {
		isize = itemSize;
		size = dataset.size;
		shape = dataset.shape.clone();
		name = new String(dataset.name);
	...
	public int[] getIntegerArray(final int... pos) { // CLASS_TYPE // PRIM_TYPE
		...
		return (int[]) getObjectAbs(n); // PRIM_TYPE
	}
	...
	@Override
	public CompoundIntegerDataset getSlice(final int[] start, final int[] stop, final int[] step) {
	...
	@Override
	public CompoundIntegerDataset setSlice(final Object o, final int[] start, final int[] stop, final int[] step) {
		SliceIterator siter;
		if (o instanceof AbstractDataset) {
			...
			if (ds instanceof AbstractCompoundDataset) {
				...
				while (siter.hasNext() && oiter.hasNext()) {
					for (int i = 0; i < isize; i++)
						data[siter.index + i] = (int) ds.getElementLongAbs(oiter.index + i); // GET_ELEMENT_WITH_CAST
				}
			} else {
				while (siter.hasNext() && oiter.hasNext()) {
