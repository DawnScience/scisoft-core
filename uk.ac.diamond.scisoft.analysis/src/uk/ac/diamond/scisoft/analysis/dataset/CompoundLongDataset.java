...
public class CompoundLongDataset extends AbstractCompoundDataset {
	...
	@Override
	public CompoundLongDataset setSlice(final Object o, final int[] start, final int[] stop, final int[] step) {
		SliceIterator siter;
		if (o instanceof AbstractDataset) {
			...
			if (ds instanceof AbstractCompoundDataset) {
				...
				while (siter.hasNext() && oiter.hasNext()) {
					for (int i = 0; i < isize; i++)
						data[siter.index + i] = ds.getElementLongAbs(oiter.index + i); // GET_ELEMENT_WITH_CAST
				}
			} else {
				while (siter.hasNext() && oiter.hasNext()) {
