...
public class CompoundShortDataset extends AbstractCompoundDataset {
	 ...
	 * @param wrap
	 */
	public CompoundShortDataset(final CompoundShortDataset dataset, final boolean wrap) {
		isize = dataset.isize;
		size = dataset.size;
	 ...
	 * @param datasets
	 */
	public CompoundShortDataset(final AbstractDataset... datasets) {
	 ...
	 * @param dataset
	 */
	public CompoundShortDataset(final int itemSize, final boolean repeat, final AbstractDataset dataset) {
		isize = itemSize;
		size = dataset.size;
		shape = dataset.shape.clone();
		name = new String(dataset.name);
	...
	@Override
	public CompoundShortDataset getSlice(final int[] start, final int[] stop, final int[] step) {
	...
	@Override
	public CompoundShortDataset setByBoolean(final Object o, BooleanDataset selection) {
	...
	@Override
	public CompoundShortDataset setByIndex(final Object o, IntegerDataset index) {
		if (o instanceof AbstractDataset) {
			AbstractDataset ds = (AbstractDataset) o;
	...
	@Override
	public CompoundShortDataset setSlice(final Object o, final int[] start, final int[] stop, final int[] step) {
		SliceIterator siter;
		if (o instanceof AbstractDataset) {
			...
			if (ds instanceof AbstractCompoundDataset) {
				...
				while (siter.hasNext() && oiter.hasNext()) {
					for (int i = 0; i < isize; i++)
						data[siter.index + i] = (short) ds.getElementLongAbs(oiter.index + i); // GET_ELEMENT_WITH_CAST
				}
			} else {
				while (siter.hasNext() && oiter.hasNext()) {
					data[siter.index] = (short) ds.getElementLongAbs(oiter.index); // GET_ELEMENT_WITH_CAST
					for (int i = 1; i < isize; i++)
