package uk.ac.diamond.scisoft.analysis.delaunay_triangulation;
 ...
 * <br>
 * Testing (done in early 2005): Platform java 1.5.02 windows XP (SP2), AMD laptop 1.6G sempron CPU 512MB RAM.
 * Constructing a triangulation of 100,000 vertices takes ~ 10 seconds. point location of 100,000 points on a
 * triangulation of 100,000 vertices takes ~ 5 seconds. Note: constructing a triangulation with 200,000 vertices and
 * more requires extending java heap size (otherwise an exception will be thrown).<br>
 ...
 *         The project uses some ideas presented in the VoroGuide project, written by Klasse f?r Kreise (1996-1997), For
 *         the original applet see: http://www.pi6.fernuni-hagen.de/GeomLab/VoroGlide/ . <br>
 */
...
public class Delaunay_Triangulation {
	...
	/**
	 * creates a Delaunay Triangulation from all the points in the suggested tsin file or from a smf file (off like). if
	 * the file name is .smf - read it as an smf file as try to read it as .tsin <br>
	 * Note: duplicated points are ignored! <br>
	 * SMF file has an OFF like format (a face (f) is presented by the indexes of its points - starting from 1 - not
	 * from 0): <br>
	 * begin <br>
	 * v x1 y1 z1 <br>
	 * ... <br>
	 ...
	 * 
	 * @return the number of vertices in the triangulation (duplicates are ignore - set size).
	 */
	public int size() {
		if (_vertices == null) {
			return 0;
	...
	/**
	 * insert the point to this Delaunay Triangulation. Note: if p is null or already exist in this triangulation p is
	 * ignored.
	 * 
	 * @param p
	...
	/**
	 * Calculates a Voronoi cell for a given neighborhood in this triangulation. A neighborhood is defined by a triangle
	 * and one of its corner points. By Udi Schneider
	 * 
	 * @param triangle
	 ...
	 * 
	 * @return iterator to all triangles involved in the last update of the triangulation NOTE: works ONLY if the are
	 *         triangles (it there is only a half plane - returns an empty iterator
	 */
	...
	/*
	 * SMF file has an OFF like format (a face (f) is presented by the indexes of its points - starting from 1, and not
	 * from 0): begin v x1 y1 z1 ... v xn yn zn f i11 i12 i13 ... f im1 im2 im3 end
	 */
	...
	/**
	 * finds the triangle the query point falls in, note if out-side of this triangulation a half plane triangle will be
	 * returned (see contains), the search has expected time of O(n^0.5), and it starts form a fixed triangle
	 * (this.startTriangle),
	 * 
	 * @param p
	 *            query point
	...
	/**
	 * finds the triangle the query point falls in, note if out-side of this triangulation a half plane triangle will be
	 * returned (see contains). the search starts from the the start triangle
	 * 
	 * @param p
	 *            query point
	 * @param start
	...
	/*
	 * find triangle to be added to the triangulation By: Doron Ganel & Eyal Roth
	 */
	 ...
	 * @return true iff (x,y) falls inside this triangulation (in its 2D convex hull).
	 */
	 ...
	 * @param q
	 *            Query point
	 * @return the q point with updated Z value (z value is as given the triangulation).
	 */
