...
class CrystalSystem {
	 ...
	 * @return All the possible crystal system names
	 */
	...
	public static CrystalSystem getSystemFromGroupNumber(int groupNumber) {
		for (int i = 0; i < nSystems; i++) {
			if (groupNumber >= lowestGroups[i] && groupNumber <= highestGroups[i]) {
				return CrystalSystem.get(i);
			}
		}
		return null;
	}
	
	public int getFirstSpaceGroupNumber() {
		return lowestGroups[this.getOrdinal()];
	}
	...
	private final static int[] lowestGroups = {1, 3, 16, 75, 143, 168, 195, 231};
	private final static int[] highestGroups = {2, 15, 74, 142, 167, 194, 230, 237};
