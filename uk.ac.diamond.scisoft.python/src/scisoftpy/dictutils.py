...
class ListDict(object):
    ...
    def __init__(self, data=[], warn=True, lock=False, interactive=True):
        ...
        if isinstance(data, dict):
            data = [ i for i in list(data.items()) ]
        if interactive:
    ...
    def __iter__(self):
        return iter(self.__odict__.keys())

    def keys(self):
        return list(self.__odict__.keys())
    ...
    def values(self):
        return list(self.__odict__.values())

    def items(self):
        return list(self.__odict__.items())

    def iterkeys(self):
        return iter(self.__odict__.keys())

    def itervalues(self):
        return iter(self.__odict__.values())

    def iteritems(self):
        return iter(self.__odict__.items())
...
def _test_make_safe():
    # test make_safe
    data = [ ('a', 0), ('a', 1), ('a.b', -2), ('a$asd923', -4), ('a£b£.123', -7) ]
