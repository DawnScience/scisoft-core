...
public abstract class AbstractTracker {
	...
	public void interpolationRoutine() {
		...
		if (isTheTrackingMarkerNot3) {
			...
			drm.getFms().get(selection).setRoiLocation(location);
	...
	public void initialiseTracker() {
		...
		try {
			...
			ITrackerFactoryObject itfo = new ITrackerFactoryObject(input, initialLocation,
		...
		drm.addTrackerLocationList(frame.getImageNumber(), initialLocation);
		drm.setTrackerCoordinates(location);
	...
	public void followUp() {
		...
		try {
			...
			if (location == null) {
				double[] l = TrackerLocationInterpolation.trackerInterpolationInterpolator0(
		...
		if (drm.getInputForEachDat()[frame.getDatNo()] == null) {
			...
			if (drm.getInitialTracker() == null) {
				...
				if (seedLocation == null) {
					...
					System.out.println(
							"extrapolation inside restart method, selection:        " + k + "      seedlocation [0]:   "
									+ seedLocation[0] + "    seedlocation [1]:   " + seedLocation[1]);
					trackedPosition = false;
				} else {
				...
				try {
					...
					trackerInitialCopy.initialize(input, location,
							TrackingMethodology.toTT(frame.getTrackingMethodology()));
			...
			if (location == null) {
				...
				try {
					ITrackerFactoryObject itfo = new ITrackerFactoryObject(input, location,
							TrackingMethodology.toTT(frame.getTrackingMethodology()));
			...
			try {
				...
				if (location != null) {
					...
					tracker = BoofCVImageTrackerServiceCreator.createImageTrackerService();
					tracker.initialize(drm.getInputForEachDat()[frame.getDatNo()], seedLocation,
							TrackingMethodology.toTT(frame.getTrackingMethodology()));
	...
	private double[] sanityCheck(double[] kROI) {
		...
		try {
			...
			if (kMinusOneROI != null & kMinusTwoROI != null) {
				...
				double[] calculatedPosition = TrackerLocationInterpolation.trackerInterpolationInterpolator0(
						drm.getTrackerLocationList(), drm.getSortedX(), drm.getInitialLenPt()[0], selection);

				double xCalculatedStep = calculatedPosition[0] - kMinusOneROI[0];
				double yCalculatedStep = calculatedPosition[1] - kMinusOneROI[1];

				double xyCalculatedStepLength = Math.pow((Math.pow(xCalculatedStep, 2) + Math.pow(yCalculatedStep, 2)),
						0.5);

				double angleOfDivertionFromPreviousTrack = Math
						.abs((Math.PI / 2) - Math.atan(xPreviousStep / yPreviousStep) - Math.atan(yStep / xStep));
				...
				if (angleOfDivertionFromPreviousTrack > (Math.PI / 18) || xyStepLength > 1.1 * xyCalculatedStepLength) {
					...
					trackerInitialCopy.initialize(input, kROI,
							TrackingMethodology.toTT(frame.getTrackingMethodology()));
