...
public class AgnosticTrackerWithFrames {
	...
	public void TwoDTracking0(DirectoryModel drm, int trackingMarker, int k, int selection) {
		...
		if (frame.getTrackingMethodology() != TrackingMethodology.TrackerType1.SPLINE_INTERPOLATION
			...
			if (trackingMarker == 0) {
				if (drm.getInputForEachDat()[frame.getDatNo()] == null) {
					...
					debug("TwoDTracking0 initialLocation[0]:  " + initialLocation[0]
							+ "TwoDTrackign3 initialLocation[1]:  " + initialLocation[1]);

					try {
						tracker.initialize(input, initialLocation,
								TrackingMethodology.toTT(frame.getTrackingMethodology()));
						tracker1.initialize(input, initialLocation,
								TrackingMethodology.toTT(frame.getTrackingMethodology()));
					} catch (Exception e) {
					...
					drm.addTrackerLocationList(frame.getImageNumber(), location);
					...
					try {
						...
						if (location != null) {
							...
							tracker = BoofCVImageTrackerServiceCreator.createImageTrackerService();
							tracker.initialize(drm.getInputForEachDat()[frame.getDatNo()], drm.getTrackerCoordinates(),
									TrackingMethodology.toTT(frame.getTrackingMethodology()));
							location = tracker.track(input);
							if (location != null) {
								drm.setTrackerCoordinates(location);
						...
						if (location == null) {
							location = TrackerLocationInterpolation.trackerInterpolationInterpolator0(
									drm.getTrackerLocationList(), drm.getSortedX(), drm.getInitialLenPt()[0],
									selection);
						}
				...
				if (drm.getInputForEachDat()[frame.getDatNo()] == null) {
					...
					if (drm.getInitialTracker() == null) {
						...
						try {
							...
							debug("TwoDTrackign3 interpolated location for trackingMarker = 2 ; location[0]:  "
									+ location[0] + "    TwoDTrackign3 location[1]:  " + location[1]);

							tracker.initialize(drm.getInputForEachDat()[frame.getDatNo()], location,
									TrackingMethodology.toTT(frame.getTrackingMethodology()));
					...
					if (location == null) {
						location = TrackerLocationInterpolation.trackerInterpolationInterpolator0(
								drm.getTrackerLocationList(), drm.getSortedX(), drm.getInitialLenPt()[0], selection);
					}
			...
			if (isTheTrackingMarkerNot3) {
				...
				int[] localLocation = new int[] { localPt[0], localPt[1], (localPt[0] + localLen[0]), (localPt[1]),
						localPt[0], localPt[1] + localLen[1], (localPt[0] + localLen[0]), (localPt[1] + localLen[1]) };

				drm.addLocationList(frame.getDatNo(), drm.getNoOfImagesInDatFile(frame.getDatNo()), k, localLocation);
		...
		else if (frame.getTrackingMethodology() == TrackingMethodology.TrackerType1.SPLINE_INTERPOLATION) {
			/////// start the interpolation tracker

			try {
				if (isTheTrackingMarkerNot3) {
					len = new int[] { (int) Math.round(drm.getInterpolatedLenPts().get(selection)[0][0]),
							(int) Math.round(drm.getInterpolatedLenPts().get(selection)[0][1]) };
					pt = new int[] { (int) Math.round(drm.getInterpolatedLenPts().get(selection)[1][0]),
							(int) Math.round(drm.getInterpolatedLenPts().get(selection)[1][1]) };

					double[] localLocation = new double[] { (double) pt[0], (double) pt[1], (double) (pt[0] + len[0]),
							(double) (pt[1]), (double) pt[0], (double) pt[1] + len[1], (double) (pt[0] + len[0]),
							(double) (pt[1] + len[1]) };

					drm.addLocationList(frame.getDatNo(), drm.getNoOfImagesInDatFile(frame.getDatNo()), k,
							localLocation);
					drm.addLocationList(frame.getDatNo(), drm.getNoOfImagesInDatFile(frame.getDatNo()), k,
							localLocation);
					...
					double[] localLocation = LocationLenPtConverterUtils
							.lenPtToLocationConverter(new int[][] { localLen, localPt });
	...
	////////////////////////////// The following is only for use with seed
	////////////////////////////// locations//////////

	public void TwoDTracking1(DirectoryModel drm, int trackingMarker, int k, double[] seedLocation, int selection) {
		...
		fm.getTrackingMethodology() != TrackingMethodology.TrackerType1.SPLINE_INTERPOLATION
			...
			if (trackingMarker == 0) {
				if (drm.getInputForEachDat()[fm.getDatNo()] == null) {
					...
					try {
						...
						if (location != null) {
							drm.setTrackerCoordinates(location);
							drm.addTrackerLocationList(fm.getImageNumber(), location);
							drm.setTracker(tracker);
							...
							tracker = BoofCVImageTrackerServiceCreator.createImageTrackerService();
							tracker.initialize(drm.getInputForEachDat()[fm.getDatNo()], drm.getTrackerCoordinates(),
									TrackingMethodology.toTT(fm.getTrackingMethodology()));
							location = tracker.track(input);
							if (location != null) {
								drm.setTrackerCoordinates(location);
				...
				if (drm.getInputForEachDat()[fm.getDatNo()] == null) {
					...
					if (drm.getInitialTracker() == null) {
						...
						try {
							...
							tracker.initialize(drm.getInputForEachDat()[fm.getDatNo()], location,
									TrackingMethodology.toTT(fm.getTrackingMethodology()));
			...
			if (isTheTrackingMarkerNot3) {
				...
				drm.addLocationList(fm.getDatNo(), drm.getNoOfImagesInDatFile(fm.getDatNo()), k, localLocation);
		...
		fm.getTrackingMethodology() == TrackingMethodology.TrackerType1.SPLINE_INTERPOLATION) {
			...
			try {
				if (isTheTrackingMarkerNot3) {
					...
					double[] localLocation = LocationLenPtConverterUtils
							.lenPtToLocationConverter(new int[][] { len, pt });

					int a = drm.getNoOfImagesInDatFile(fm.getDatNo());

					drm.addLocationList(fm.getDatNo(), drm.getNoOfImagesInDatFile(fm.getDatNo()),
							fm.getNoInOriginalDat(), localLocation);
					...
					int[] localPt = drm.getInitialLenPt()[1];
					// int[] localLen = new int[] {(int) ((int)location[2] - (int)location[0]),
					// (int) ((int)location[5] -(int)location[1])};
					int[] localLen = drm.getInitialLenPt()[0];
	...
	public void TwoDTracking3(DirectoryModel drm, int trackingMarker, int k, int selection) {
		...
		if (fm.getTrackingMethodology() != TrackingMethodology.TrackerType1.SPLINE_INTERPOLATION
				&& fm.getTrackingMethodology() != TrackingMethodology.TrackerType1.USE_SET_POSITIONS) {
			isTheTrackingMarkerNot3 = true;
			...
			if (trackingMarker == 0) {
				if (drm.getInputForEachDat()[fm.getDatNo()] == null) {
					...
					debug("TwoDTrackign3 initialLocation[0]:  " + initialLocation[0]
							+ "TwoDTrackign3 initialLocation[1]:  " + initialLocation[1]);
					...
					drm.setTrackerCoordinates(new double[] { initialLocation[0], initialLocation[1], initialLocation[2],
							initialLocation[3], initialLocation[4], initialLocation[5], initialLocation[6],
							initialLocation[7] });
					drm.getInputForEachDat()[fm.getDatNo()] = (input);
					location = initialLocation;
				...
				if (drm.getInputForEachDat()[fm.getDatNo()] == null) {
					...
					debug("trackign mrker 2; TwoDTrackign3 initialLocation[0]:  " + initialLocation[0]
							+ "    TwoDTrackign3 initialLocation[1]:  " + initialLocation[1]);

					location = TrackerLocationInterpolation.trackerInterpolationInterpolator0(
							drm.getTrackerLocationList(), drm.getSortedX(), drm.getInitialLenPt()[0], selection);

					debug("TwoDTrackign3 interpolated location for trackingMarker = 2 ; location[0]:  " + location[0]
							+ "    TwoDTrackign3 location[1]:  " + location[1]);
					...
					if (drm.getInitialTracker() != null) {
						...
						try {
							...
							tracker.initialize(drm.getInitialDatasetForEachDat()[fm.getDatNo()], location,
									TrackingMethodology.toTT(fm.getTrackingMethodology()));
						} catch (Exception e) {
							debug("Failed to initialise tracker in TwoDTracking3 ");
						}
			...
			if (isTheTrackingMarkerNot3) {
				...
				drm.addLocationList(a, b, k, localLocation);
		...
		else if (fm.getTrackingMethodology() == TrackingMethodology.TrackerType1.SPLINE_INTERPOLATION) {
