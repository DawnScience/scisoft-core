...
public class ClosestNoFinder {
	...
	public static int closestNoWithLocation(double myNum, Dataset numbers, ArrayList<double[]> locationList) {
		...
		for (int d = 0; d < numbers.getSize(); d++) {
			if (test.equals(Arrays.equals(locationList.get(d), test) == false)
					& Arrays.equals(locationList.get(d), null) == false) {
				idx = d;
		...
		for (int c = 0; c < numbers.getSize(); c++) {
			...
			if ((cdistance < distance) & (Arrays.equals(locationList.get(c), test) == false)
					& Arrays.equals(locationList.get(c), null) == false) {
				idx = c;
				distance = cdistance;
			}
		}
	...
	public static int closestNoWithoutDone(double myNum, Dataset numbers, String[] doneArray,
	...
	public static boolean full(boolean[] stringArray, String test) {
	...
	public static int closestNoWithDone(double myNum, Dataset numbers, String[] doneArray, int[] filepathSortedArray) {
