...
public class RodObjectNexusUtils {
	...
	public RodObjectNexusUtils(RodObjectNexusBuilderModel model) {
		...
		for (int imageFilepathNo = 0; imageFilepathNo < noImages; imageFilepathNo++) {
			...
			nxData.addAttribute(TreeFactory.createAttribute("Is Good Point", f.isGoodPoint()));
			...
			nxData.addAttribute(
					TreeFactory.createAttribute("Fit_Power", AnalaysisMethodologies.toString(f.getFitPower())));
			...
			nxData.addAttribute(TreeFactory.createAttribute("Tracker_Type",
					TrackingMethodology.toString(f.getTrackingMethodology())));
			nxData.addAttribute(TreeFactory.createAttribute("Background_Methodology",
					AnalaysisMethodologies.toString(f.getBackgroundMethdology())));
		...
		try {
			...
			try {
				...
				System.out.println(
						"This error occured when attempting to close the NeXus file: " + closingerror.getMessage());

			}
			System.out.println(
					"This error occured when attempting to open the NeXus file: " + nexusFileError.getMessage());
		}
	}

	private void geometricalParameterWriter(GeometricParametersModel gm, GroupNode entry) {
